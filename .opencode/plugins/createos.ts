...
import { shortId, shellQuote, joinPath } from "../../packages/opencode-plugin/src/util.ts"

interface ActiveSandbox { sandboxId: string; cwd: string; shape: string }
...
export const CreateOSPlugin: Plugin = async ({ project, client, $, directory }) => {
    ...
    if (!initPromise) {
      initPromise = (async () => {
        if (!(await cli.isCreateOSInstalled($))) {
          if (!(await cli.autoInstallCLI($))) throw new Error("Failed to install CreateOS CLI. Run: curl -sfL https://raw.githubusercontent.com/NodeOps-app/createos-cli/main/install.sh | sh")
        }
        if (!(await cli.isLoggedIn($))) throw new Error("Not logged in to CreateOS. Run: createos login")
  ...
  function sid() { return active?.sandboxId ?? "pending" }
...
--- End CreateOS ---`
  ...
  const tools: Record<string, any> = {
    ...
    sandbox_exec: tool({
      description: "Run a shell command inside the CreateOS sandbox. Use this for ALL shell commands — installing packages, running scripts, compiling code, starting servers, checking system info. NEVER use the built-in bash tool.",
      args: {
        command: tool.schema.string().describe("Shell command to run inside the sandbox"),
        sandbox_id: tool.schema.string().optional().describe("Target sandbox ID (defaults to the active sandbox)"),
      },
      async execute(args) {
    ...
    sandbox_info: tool({
      ...
      async execute(args) {
        ...
        return [
          ...
          `IP: ${info.ip ?? "n/a"}`, `Shape: ${(info as any).shape ?? "n/a"}`,
    ...
    sandbox_create: tool({
      description: "Create an additional sandbox. Use when the user needs multiple sandboxes — multi-node clusters, separate database servers, microservice setups.",
      args: {
        shape: tool.schema.string().optional().describe("VM size (default: s-2vcpu-2gb). Use sandbox_shapes to see options."),
        ...
        networks: tool.schema.array(tool.schema.string()).optional().describe("Network names to join at creation"),
      },
      async execute(args) {
        await ensureSandbox()
        const sb = await cli.createSandbox($, { shape: args.shape, rootfs: args.rootfs, name: args.name, networks: args.networks, ingress: true })
    ...
    sandbox_pause: tool({
      ...
      args: { sandbox_id: tool.schema.string().optional().describe("Sandbox ID (defaults to current)") },
      async execute(args) {
        const s = await ensureSandbox()
    ...
    sandbox_fork: tool({
      description: "Clone a paused sandbox into a brand-new sandbox with the same state. Source must be paused first.",
      args: {
        sandbox_id: tool.schema.string().optional().describe("Sandbox ID to fork (defaults to current)"),
      },
      async execute(args) {
        const s = await ensureSandbox()
    ...
    sandbox_firewall: tool({
      ...
      args: {
        rules: tool.schema.array(tool.schema.string()).describe('Allowed domains or IPs (e.g. ["pypi.org", "1.1.1.1:53"]). Empty = allow all.'),
        sandbox_id: tool.schema.string().optional().describe("Sandbox ID (defaults to current)"),
      },
      async execute(args) {
        const s = await ensureSandbox()
        await cli.editSandbox($, args.sandbox_id ?? s.sandboxId, { egress: args.rules })
        return args.rules.length ? `Firewall set: ${args.rules.join(", ")}` : "Firewall cleared — all outbound allowed."
      },
    }),
    ...
    sandbox_shapes: tool({
      ...
      args: {},
      async execute() { return JSON.stringify(await cli.listShapes($), null, 2) },
    }),

    sandbox_images: tool({
      description: "List available base images (rootfs) for sandbox creation.",
      args: {},
      async execute() { return JSON.stringify(await cli.listRootfs($), null, 2) },
    }),

    // === Ports & Connectivity ===
    sandbox_preview_url: tool({
      description: "Get the public HTTPS URL for a port served inside the sandbox. Use after starting a server to give the user a clickable link.",
      args: { port: tool.schema.number().describe("The port the server listens on") },
      async execute(args) {
        const s = await ensureSandbox()
        const info = await cli.getSandbox($, s.sandboxId)
        if (!info.ingress_url_template) return "Ingress not enabled. Use sandbox_ingress to enable it first."
        return `Preview URL for port ${args.port}: ${info.ingress_url_template.replace("<port>", String(args.port))}`
      },
    }),

    sandbox_tunnel: tool({
      description: "Forward a sandbox port to localhost on the user's machine. No setup needed. Prefer sandbox_preview_url for sharing.",
      args: {
        remote_port: tool.schema.number().describe("Port inside the sandbox"),
        local_port: tool.schema.number().optional().describe("Local port (defaults to same as remote)"),
      },
      async execute(args) {
        const s = await ensureSandbox()
    ...
    sandbox_sync: tool({
      description: "Mount/sync a local directory from the user's machine into the sandbox. Bidirectional by default.",
      args: {
        local_dir: tool.schema.string().describe("Absolute path to the local directory"),
        remote_dir: tool.schema.string().describe("Absolute path inside the sandbox (e.g. /root/project)"),
        mode: tool.schema.string().optional().describe('Sync mode: "two-way" (default), "one-way", or "mirror"'),
        exclude: tool.schema.array(tool.schema.string()).optional().describe('Patterns to exclude (e.g. ["node_modules", "*.log"])'),
      },
      async execute(args) {
        const s = await ensureSandbox()
        const r = await cli.startSync($, s.sandboxId, args.local_dir, args.remote_dir, { mode: args.mode, exclude: args.exclude })
    ...
    sandbox_network_create: tool({
      ...
      async execute(args) { const n = await cli.createNetwork($, args.name); return `Network created: ${n.name} (${n.id})` },
    }),
    ...
    sandbox_network_attach: tool({
      ...
      args: { name: tool.schema.string().describe("Network name or ID") },
      async execute(args) { const s = await ensureSandbox(); await cli.attachNetwork($, s.sandboxId, args.name); return `Attached to "${args.name}".` },
    }),
    sandbox_network_detach: tool({
      description: "Detach the current sandbox from a private network.",
      args: { name: tool.schema.string().describe("Network name or ID") },
      async execute(args) { const s = await ensureSandbox(); await cli.detachNetwork($, s.sandboxId, args.name); return `Detached from "${args.name}".` },
    }),
    sandbox_network_delete: tool({
      description: "Delete a private network. Detach all sandboxes first.",
      args: { name: tool.schema.string().describe("Network name or ID") },
      async execute(args) { await cli.deleteNetwork($, args.name); return `Network "${args.name}" deleted.` },
    }),
    ...
    sandbox_disk_create: tool({
      ...
      async execute(args) {
        const d = await cli.createDisk($, { name: args.name, bucket: args.bucket, endpoint: args.endpoint, accessKey: args.access_key, secretKey: args.secret_key, region: args.region, pathStyle: args.path_style })
    ...
    sandbox_disk_list: tool({
      ...
      async execute() {
        ...
        return disks.map(d => `${d.name} (${d.id})${d.config?.bucket ? ` · bucket: ${d.config.bucket}` : ""}`).join("\n")
      },
    }),
    sandbox_disk_show: tool({
      ...
      async execute(args) { return JSON.stringify(await cli.getDisk($, args.name), null, 2) },
    }),
    sandbox_disk_delete: tool({
      description: "Delete a registered disk. Must be detached from all sandboxes first.",
      args: { name: tool.schema.string().describe("Disk name or ID") },
      async execute(args) { await cli.deleteDisk($, args.name); return `Disk "${args.name}" deleted.` },
    }),
    ...
    sandbox_device_register: tool({
      description: "Register the user's machine as a device for direct sandbox access. One-time setup. Requires wireguard-tools.",
      args: { name: tool.schema.string().optional().describe("Device name (defaults to hostname)") },
      async execute(args) {
        const devs = await cli.listDevices($)
        if (devs.length) return `Device already registered: ${devs[0].name} (${devs[0].client_ip ?? "n/a"})`
    ...
    sandbox_device_status: tool({
      ...
      async execute() {
        ...
        return devs.map(d => `${d.name} · ${d.id ?? d.device_id} · IP: ${d.client_ip ?? "n/a"}`).join("\n")
      },
    }),
    sandbox_vpn_up: tool({
      description: "Returns the command the user must run in a separate terminal to start the VPN tunnel. Requires sudo.",
      args: {},
      async execute() {
    ...
    sandbox_device_attach: tool({
      ...
      async execute(args) {
        const devs = await cli.listDevices($)
        if (!devs.length) throw new Error("No device registered. Use sandbox_device_register first.")
