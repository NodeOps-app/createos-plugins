...
function execCmd(cmd: string): { code: number; stdout: string; stderr: string } {
  ...
  try {
    const stdout = execSync(cmd, { encoding: 'utf-8', timeout: 120000, stdio: ['pipe', 'pipe', 'pipe'] })
...
export async function ensureTempKey($: any): Promise<string> {
  if (tempKeyPath) return tempKeyPath
  const shellCmd = 'dir=$(mktemp -d) && ssh-keygen -t ed25519 -f "$dir/id_sync" -N "" -q && echo "$dir/id_sync"'
  const res = execCmd(shellCmd)
...
export async function cleanupTempKey($: any): Promise<void> {
...
export async function detachDisk(
  ...
  const res = await run($, ['sandbox', 'disk', 'detach', sandboxId, diskIdOrName, mountPath, '--yes'])
