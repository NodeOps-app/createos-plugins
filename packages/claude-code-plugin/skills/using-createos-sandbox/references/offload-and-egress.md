...
**Disk fills fast.** `pip install --all-extras` or an unconstrained `uv sync` can pull CUDA and torch wheels measured in gigabytes and hit `No space left on device` on a small box. Install only what the job needs; `cos up -d 40960` (MiB) raises the ceiling on a project box — the default is 10 GiB, which Docker-heavy apps that pull extra images outgrow.
