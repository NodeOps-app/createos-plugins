...
A [Claude Code](https://docs.claude.com/en/docs/claude-code) plugin that gives Claude a skill + 23 slash commands driving the authed [`createos`](https://createos.sh) CLI. Work runs in disposable [CreateOS](https://createos.sh) Sandboxes — roughly 200 ms from create to your first command — that self-destruct when done.
...
| Piece              | Path                                                     | Role                                                                                                        |
| ------------------ | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| **Slash commands** | `commands/*.md`                                          | 23 commands (`offload`, `exec`, `fanout`, `matrix`, `shell`, …), each a thin wrapper that calls `scripts/cos`                 |
...
> **Flag order:** flags (`-s/-r/-e/-p/-E/-o/-x/-v/-K`) come **before** the positional `<dir> <cmd>`.
