...
interface SessionEntryData { sandboxId: string; cwd: string }
interface ActiveSandbox { sandboxId: string; cwd: string }

/** The user's local working directory (where they launched Pi from). */
const hostCwd = process.cwd()

export default function (pi: ExtensionAPI) {
  pi.registerFlag('createos', { description: 'Run tools inside a CreateOS sandbox', type: 'boolean' })
  ...
  pi.registerFlag('network', { description: 'Network(s) to join (comma-separated)', type: 'string' })
  ...
  pi.registerCommand('sandbox', {
    ...
    handler: async (_args, ctx) => {
      if (!active) { ctx.ui.notify('No CreateOS sandbox is active. Launch Pi with --createos.', 'warning'); return }
      try {
      ...
      } catch (err) { ctx.ui.notify(`Failed: ${errorMessage(err)}`, 'error') }
    },
  })
  ...
  pi.registerCommand('network', {
    ...
    handler: async (args, ctx) => {
      if (!active) { ctx.ui.notify('No sandbox active. Launch with --createos.', 'warning'); return }
      const parts = (args ?? '').trim().split(/\s+/)
      const sub = parts[0], arg = parts.slice(1).join(' ').trim()
      try {
        switch (sub) {
          case 'create':
            if (!arg) { ctx.ui.notify('Usage: /network create <name>', 'warning'); return }
            const net = await cli.createNetwork(pi, arg)
            ctx.ui.notify(`Network created: ${net.name} (${net.id})`, 'info'); break
          case 'ls': case 'list':
            const nets = await cli.listNetworks(pi)
            if (!nets.length) { ctx.ui.notify('No networks. Create with /network create <name>', 'info'); return }
            ctx.ui.notify(`Networks:\n${nets.map((n) => `  ${n.name} (${n.id}) · ${n.member_count ?? 0} members`).join('\n')}`, 'info'); break
          case 'show':
            if (!arg) { ctx.ui.notify('Usage: /network show <name|id>', 'warning'); return }
            const detail = await cli.getNetwork(pi, arg)
            const lines = [`Network: ${detail.name} (${detail.id})`]
            if (detail.members?.length) {
              lines.push('Members:')
              for (const m of detail.members) lines.push(`  ${m.sandbox_id} · ${m.status} · ${m.ip}${m.name ? ` · ${m.name}` : ''}`)
            } else lines.push('No members')
            ctx.ui.notify(lines.join('\n'), 'info'); break
          case 'rm': case 'delete':
            if (!arg) { ctx.ui.notify('Usage: /network rm <name|id>', 'warning'); return }
            if (!(await ctx.ui.confirm('Delete network', `Delete "${arg}"?`))) return
            await cli.deleteNetwork(pi, arg)
            ctx.ui.notify(`Network "${arg}" deleted`, 'info'); break
          case 'attach':
            if (!arg) { ctx.ui.notify('Usage: /network attach <name|id>', 'warning'); return }
            await cli.attachNetwork(pi, active.sandboxId, arg)
            ctx.ui.notify(`Attached sandbox to network "${arg}"`, 'info'); break
          case 'detach':
            if (!arg) { ctx.ui.notify('Usage: /network detach <name|id>', 'warning'); return }
            await cli.detachNetwork(pi, active.sandboxId, arg)
            ctx.ui.notify(`Detached sandbox from network "${arg}"`, 'info'); break
          default:
            ctx.ui.notify('/network <create|ls|show|rm|attach|detach> [args]', 'info')
        }
      } catch (err) { ctx.ui.notify(`Network error: ${errorMessage(err)}`, 'error') }
    },
  })
  ...
  pi.registerCommand('device', {
    ...
    handler: async (args, ctx) => {
      const parts = (args ?? '').trim().split(/\s+/)
      const sub = parts[0] || 'status', arg = parts.slice(1).join(' ').trim()
      try {
        switch (sub) {
          case 'status': {
            const devs = await cli.listDevices(pi)
            if (!devs.length) {
              ctx.ui.notify('No device registered.\n\n  createos sb devices register   # one-time\n  createos sb vpn up              # connect', 'info')
              return
            }
            const lines = devs.map((d) => `  ${d.name} · ${d.id ?? d.device_id} · IP: ${d.client_ip ?? 'n/a'}`)
            ctx.ui.notify(`Devices:\n${lines.join('\n')}\n\nRun \`createos sb vpn up\` to connect.`, 'info'); break
          }
          case 'attach': {
            if (!arg) { ctx.ui.notify('Usage: /device attach <network>', 'warning'); return }
            const devs = await cli.listDevices(pi)
            if (!devs.length) { ctx.ui.notify('No device. Run: createos sb devices register', 'warning'); return }
            await cli.attachDeviceToNetwork(pi, devs[0].id ?? devs[0].device_id!, arg)
            ctx.ui.notify(`Device attached to "${arg}". Run \`createos sb vpn up\` to connect.`, 'info'); break
          }
          case 'detach': {
            if (!arg) { ctx.ui.notify('Usage: /device detach <network>', 'warning'); return }
            const devs = await cli.listDevices(pi)
            if (!devs.length) { ctx.ui.notify('No device registered.', 'warning'); return }
            await cli.detachDeviceFromNetwork(pi, devs[0].id ?? devs[0].device_id!, arg)
            ctx.ui.notify(`Device detached from "${arg}".`, 'info'); break
          }
          default:
            ctx.ui.notify('/device <status|attach|detach> [network]', 'info')
        }
      } catch (err) { ctx.ui.notify(`Device error: ${errorMessage(err)}`, 'error') }
    },
  })
  ...
  pi.on('session_start', async (event, ctx) => {
    ...
    try {
      ...
      const sandbox = await cli.createSandbox(pi, {
        shape, rootfs, ingress: true, networks,
      ...
      if (persisted) pi.appendEntry(SESSION_ENTRY, { sandboxId: sandbox.id, cwd } as SessionEntryData)
      ...
      active = null
      if (createdId) try { await cli.destroySandbox(pi, createdId) } catch {}
  ...
  pi.on('session_shutdown', async (event, _ctx) => {
    ...
    try { await cleanupTempKey(pi) } catch {}

    const persisted = _ctx.sessionManager.getSessionFile() !== undefined
    if (!persisted) {
      try { await cli.destroySandbox(pi, current.sandboxId) } catch {}
    }
  })
}
