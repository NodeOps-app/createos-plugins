...
export async function forkSandbox(pi: ExtensionAPI, id: string, opts?: { paused?: boolean }): Promise<SandboxInfo> {
...
export async function getBandwidth(pi: ExtensionAPI, id: string): Promise<{ quota_bytes: number; used_bytes: number; remaining_bytes: number; capped: boolean }> {
  // bandwidth is part of sandbox get response
  const info = await getSandbox(pi, id)
  return (info as any).bandwidth ?? { quota_bytes: 0, used_bytes: 0, remaining_bytes: 0, capped: false }
}
...
export async function startTunnel(
  ...
  const args = ['sandbox', 'tunnel', '--remote', String(remotePort), '--local', String(local), sandboxId]
...
export async function startSync(
  ...
  const args = ['sandbox', 'sync', '--local', localDir, '--remote', remoteDir, '--yes', '-i', keyPath]
...
export async function pushFile(pi: ExtensionAPI, id: string, content: string, remotePath: string): Promise<void> {
  ...
  const res = await run(pi, [
    'sandbox', 'exec', id, '--',
    'sh', '-c', `echo '${b64}' | base64 -d > '${remotePath.replace(/'/g, `'\\''`)}'`,
  ])
...
export async function attachNetwork(pi: ExtensionAPI, sandboxId: string, netIdOrName: string): Promise<void> {
  const res = await run(pi, ['sandbox', 'network', 'attach', sandboxId, netIdOrName])
  if (res.code !== 0) throw new CLIError('network attach', res)
}

export async function detachNetwork(pi: ExtensionAPI, sandboxId: string, netIdOrName: string): Promise<void> {
...
export async function createDisk(
  pi: ExtensionAPI,
  opts: { name: string; bucket: string; endpoint: string; accessKey: string; secretKey: string; region?: string; pathStyle?: boolean },
): Promise<DiskInfo> {
  const args = ['-o', 'json', 'sandbox', 'disk', 'create', opts.name,
    '--bucket', opts.bucket, '--endpoint', opts.endpoint,
    '--access-key', opts.accessKey, '--secret-key', opts.secretKey,
  ]
...
export async function attachDisk(pi: ExtensionAPI, sandboxId: string, diskIdOrName: string, mountPath: string): Promise<void> {
...
export async function detachDisk(pi: ExtensionAPI, sandboxId: string, diskIdOrName: string, mountPath: string): Promise<void> {
  const res = await run(pi, ['sandbox', 'disk', 'detach', sandboxId, diskIdOrName, mountPath, '--yes'])
...
export async function attachDeviceToNetwork(pi: ExtensionAPI, deviceId: string, netIdOrName: string): Promise<void> {
...
export async function detachDeviceFromNetwork(pi: ExtensionAPI, deviceId: string, netIdOrName: string): Promise<void> {
