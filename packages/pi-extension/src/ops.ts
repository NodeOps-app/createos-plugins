...
export function createBashOps(pi: ExtensionAPI, sandboxId: string, cwdOverride?: string): BashOperations {
  return {
...
export function createEditOps(pi: ExtensionAPI, sandboxId: string): EditOperations {
  return {
    ...
    writeFile: (path, content) => pushFile(pi, sandboxId, content, path),
    access: async (path) => {
      const { exitCode } = await run(pi, sandboxId, `test -r ${shellQuote(path)} && test -w ${shellQuote(path)}`)
...
export function createLsOps(pi: ExtensionAPI, sandboxId: string): LsOperations {
  return {
    ...
    stat: async (path) => {
      ...
      const { stdout, exitCode } = await run(pi, sandboxId, `test -e ${q} || exit 1; test -d ${q} && echo dir || echo other`)
